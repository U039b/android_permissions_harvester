...
def _extract_permissions(aosp_root_dir):
    permissions = {}
...
def _extract_permission_groups(aosp_root_dir):
    groups = {}
    manifest = os.path.join(aosp_root_dir, 'core/res/AndroidManifest.xml')
    xml = parse(manifest)
...
def _extract_drawable(aosp_root_dir, elements):
    for e in elements:
...
def _extract_string_prt(translation_file, elements):
...
def extract_groups_and_permissions(aosp_root_dir, locale, output_dir):
    ...
    permissions = _extract_permissions(aosp_root_dir)
    permissions = _extract_string_prt(translation_file, permissions)

    groups = _extract_permission_groups(aosp_root_dir)
    groups = _extract_string_prt(translation_file, groups)
    groups = _extract_drawable(aosp_root_dir, groups)
...
if __name__ == "__main__":
    ...
    extract_groups_and_permissions(aosp_root_dir, locale, output_dir)
